...
public class XMLExport extends Exporter {
    ...
    public void export(User[] data) throws ParserConfigurationException, TransformerException {
        ...
        for (User user : data) {
            ...
            for (String ks : user.getDataMap().keySet()) {

                Element cl = document.createElement(user.getDataMap().get(ks).getClass().getSimpleName());
                cl.appendChild(document.createTextNode(user.getDataMap().get(ks).getData().toString()));
                cd.appendChild(cl);

                Attr clAttr = document.createAttribute("name");
                clAttr.setValue(user.getDataMap().get(ks).getName());
